...
func sendTx(account *types.Account, to types.Address, token string, amount types.Balance) error {
	...
	if testLedgerFile == nil {
		testLedgerFile, err = os.OpenFile("D:\\testLedger", os.O_RDWR | os.O_CREATE | os.O_APPEND, 0777)
		if err != nil {
			fmt.Println("create test ledger file err")
			return err
		}
	}

	w := bufio.NewWriter(testLedgerFile)
	sb, _ := sendBlock.MarshalMsg(nil)
	w.Write([]byte(fmt.Sprintf("%d:", len(sb))))
	w.Write(sb)
	w.Flush()
	testLedgerFile.Sync()
	fmt.Println(sendBlock.GetHash().String())
